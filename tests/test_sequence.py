...
def test_remap_query_sets_query_to_new_query():
    # the hits now span the new query's residues, so it becomes the frame:
    # gaps dropped, insertions uppercased
    hits = [Sequence("VAST", id="p0")]
    result = Sequences(seqs=hits, aligned=True, format="a3m").remap_query("MAST", "M-ST")
    assert result.query == "MST"
    assert [s.seq for s in result.seqs] == ["VST"]

    result = Sequences(seqs=hits, aligned=True, format="a3m").remap_query("MAST", "MAStT")
    assert result.query == "MASTT"
